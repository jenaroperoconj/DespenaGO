...
import { 
  ...
  mailOutline,
  refreshOutline,
  informationCircleOutline, checkmarkOutline } from 'ionicons/icons';
import { SupabaseService } from '../core/supabase.service';

@Component({
  selector: 'app-compartir-despensa',
  templateUrl: './compartir-despensa.modal.html',
  styleUrls: ['./compartir-despensa.modal.scss'],
...
export class CompartirDespensaModal {
  ...
  constructor(
    ...
    addIcons({closeOutline,personAddOutline,searchOutline,shareOutline,shieldOutline,checkmarkCircleOutline,alertCircleOutline,helpCircleOutline,peopleOutline,personOutline,mailOutline,createOutline,trashOutline,checkmarkOutline,eyeOutline,informationCircleOutline,exitOutline,refreshOutline});
  }  async ngOnInit() {
  ...
  validarEmail() {
    if (!this.emailInvitacion || this.emailInvitacion.trim().length === 0) {
      this.emailValido = true; // No mostrar error cuando está vacío
      return;
    }
    
    // Validación básica de email con regex
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    this.emailValido = emailRegex.test(this.emailInvitacion);
  }
  
  async doRefresh(event: any) {
    try {
      console.log('🔄 Actualizando datos de colaboración...');
      ...
      this.error = `No se pudieron actualizar los datos: ${error.message || 'Error desconocido'}`;
    } finally {
      // Completar el evento de refresher
